...
import static edu.uiuc.ncsa.security.oauth_2_0.IDTokenUtil.decat;
...
public class SciTokensCommands extends CommonCommands {
    ...
    protected void setKeysHelp() {
        say("set_keys: [filename]");
        say("          Set the keys used for signing and validation in this session.");
        say("          Either supplied a fully qualified path to the file or you will be");
        say("          prompted for it. You can invoke this at any to change the keys.");
    ...
    public void create_token(InputLine inputLine) throws Exception {
        ...
        String signedToken = IDTokenUtil.createIDToken(claims, localKeys.get(localDefaultID));
    ...
    public void validate_token(InputLine inputLine) throws Exception {
        ...
        say("header=" + h);
        say("payload=" + p);
        if (IDTokenUtil.verify(h, p, x[2], keys.get(defaultKeyID))) {
            say("token valid!");
        } else {
