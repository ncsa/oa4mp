...
<html>
<head>
    <title>OA4MP Consent Page</title>
    <link rel="stylesheet"
          type="text/css"
          media="all"
...
<br clear="all"/>
<div class="main">
    <h2>Welcome to the OA4MP Consent Page</h2>
    The Client below is requesting access to your account.
    <p>
    <form action="${param.actionToTake}" method="POST">
                    ...
                    <table border=1>
                        ...
                        <tr>
                            <td> The client listed below is requesting access to your
                                ...
                                <br><br>
                                <i>Name:</i> ${param.clientName}
                                <br>
                                <i>URL:</i> ${param.clientHome}
                                <br>
                                <i>Requested Scopes:</i> ${param.clientScopes}
                            </td>
                        </tr>
                    </table>
                </td>
                <td valign="top">
                    <table width="350" cellspacing="0" cellpadding="0" align="right" border="0">
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="${param.clientHome}" STYLE="TEXT-DECORATION: NONE"><input type="submit"
                                                                                             name="cancel"
                                                                                             value="Cancel"/></a>
                            </td>
                        </tr>
                    ...
                    <!-- Close sign in table -->
                    <input type="hidden" id="status" name="${param.action}"
                           value="${actionOk}"/>
                    <input type="hidden" id="token" name="${param.tokenKey}" value="${param.authorizationGrant}"/>
                    <input type="hidden" id="page_type" name="page_type" value="consent"/>
                    <input type="hidden" id="proxy_key" name="proxy_key" value="${param.proxyKey}"/>
                    <input type="hidden" id="response_type" name="response_type" value="${param.responseType}"/>
                    <input type="hidden" id="user_code" name="user_code" value="${param.userCode}"/>
                </td>
            </tr>
        </table>
    </form>
