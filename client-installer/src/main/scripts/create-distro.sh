# This creates the distribution jar archive for OA4MP client.
...
# OA4MP_CLIENT_SOURCES = the client installer directory

DEFAULT_OA4MP_ROOT=$NCSA_DEV_INPUT/oa4mp
OA4MP_ADMIN_SOURCES=$NCSA_DEV_INPUT/oa4mp/server-admin
OA4MP_CLIENT_SOURCES=$NCSA_DEV_INPUT/oa4mp/client-installer
JAR_NAME="client-archive.jar"
# The next is where the full build places the constructed wars and jars.
OA4MP_SERVER_DEPLOY=$NCSA_DEV_OUTPUT/oa4mp

if [[  "$1" = "--help" ]];then
  echo "create-distro.sh [oa4mp_root]"
  echo "create the directory structure and populate it for OA4MP."
...
echo "OA4MP version: 6.1-rc1"  > version.txt
...
jar cf  "$JAR_NAME" bin docs etc examples lib log var version.txt
