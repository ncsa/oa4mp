 ...
 */
public abstract class AbstractCFClientLoader<T extends ClientEnvironment> extends CFDBConfigLoader<T> implements ClientLoaderInterface<T> {
    ...
    protected AbstractCFClientLoader(CFNode node) {
        super(node);
    }

    protected AbstractCFClientLoader(CFNode node, MyLoggingFacade logger) {
        super(node, logger);
    }
    ...
    protected String getCfgValue(String key) {
        CFNode cfNode = cn.getFirstNode(key);
        if(cfNode == null) {return null;}
