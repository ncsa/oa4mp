...
E.g. replace `6.2.2` with `5.3.5`. Note this must be global in all files since this is used extensively in 
...
1. Do a global replace of the SNAPSHOT tag with the version you are creating. E.g. replace `6.2.2` with `5.3.5`. Note this must be global in all files since this is used extensively in the documentation. You do not need to update anything in `$NCSA_DEV_INPUT/oa4mp/docs` which will be replaced shortly. 
