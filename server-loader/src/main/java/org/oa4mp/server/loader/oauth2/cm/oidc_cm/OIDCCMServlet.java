    ...
    public void doGet(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse) throws ServletException, IOException {
        ...
        try {
            ...
            if (isAnonymous) {
                ...
                Identifier clientID = BasicIdentifier.newID(creds[HeaderUtils.ID_INDEX]);
            ...
            if (isAnonymous) {
                ...
                if (!getOA2SE().getPermissionStore().getClients(adminClient.getIdentifier()).contains(id)) {
                    throw new IllegalAccessException("this admin client does not administer the client with ID \"" + id + "\".");
                }
  ...
  /*          if ((!isAnonymous) && adminClient.isDebugOn()) {
    ...
    protected JSONObject toJSONObject5_5(OA2Client client, boolean isGet) {
        ...
        if (client.hasJWKSURI()) {
            gotJWKSURI = true;
            json.put(OIDCCMConstants.JWKS_URI, client.getJwksURI().toString());
        }
        if (client.hasJWKS()) {
            if (gotJWKSURI) {
        ...
        if (!client.getResponseTypes().isEmpty()) {
            ...
            json.put(OIDCCMConstants.RESPONSE_TYPES, client.getResponseTypes());
        }
        if (client.isPublicClient()) {
            json.put(OIDCCMConstants.TOKEN_ENDPOINT_AUTH_METHOD, TOKEN_ENDPOINT_AUTH_NONE);
        }
        ...
        if (client.hasJWKS()) {
            json.put(OIDCCMConstants.JWKS, JSONWebKeyUtil.toJSON(client.getJWKS()));
        }
        if (client.hasJWKSURI()) {
            json.put(OIDCCMConstants.JWKS_URI, client.getJwksURI().toString());
        }
    ...
    protected void doPut(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            if (jsonRequest.containsKey(CLIENT_ID)) {
                client = (OA2Client) getOA2SE().getClientStore().get(BasicIdentifier.newID(jsonRequest.getString(CLIENT_ID)));
            } else {
            ...
            if (jsonRequest.containsKey(OIDCCMConstants.CLIENT_SECRET)) {
                // then we have to check this is a valid client. If this is missing, then
                // we are being requested to generate a new secret.
                String hashedSecret = DigestUtils.sha1Hex(jsonRequest.getString(OIDCCMConstants.CLIENT_SECRET));
            ...
            jsonRequest.remove(OIDCCMConstants.CLIENT_SECRET);
            jsonRequest.remove(OIDCCMConstants.CLIENT_ID);
    ...
    protected void doIt2(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse) throws Throwable {
        ...
        // Next, we have to construct the registration URI by adding in the client ID.
        // Spec says we can add parameters here, but not elsewhere.
        jsonResp.put(OIDCCMConstants.REGISTRATION_CLIENT_URI, registrationURI + "?" + CLIENT_ID + "=" + newClient.getIdentifierString());
    ...
    protected OA2Client updateClient(OA2Client client,
        ...
        if (jsonRequest.containsKey(OIDCCMConstants.TOKEN_ENDPOINT_AUTH_METHOD)) {
            // not required, but if present, we support exactly three nontrivial options.
            String authMethod = jsonRequest.getString(OIDCCMConstants.TOKEN_ENDPOINT_AUTH_METHOD);
            ...
            switch (authMethod) {
                ...
                case TOKEN_ENDPOINT_AUTH_PRIVATE_KEY:
                    ...
                    if (jsonRequest.containsKey(OIDCCMConstants.JWKS_URI)) {
                        client.setJwksURI(URI.create(jsonRequest.getString(OIDCCMConstants.JWKS_URI)));
                        jsonRequest.remove(OIDCCMConstants.JWKS_URI);
                        client.setServiceClient(true);
                        gotSupportedAuthMethod = true;
                    }
                    if (jsonRequest.containsKey(OIDCCMConstants.JWKS)) {
                        client.setSecret(null);
                        client.setJWKS(getJwkUtil().fromJSON(jsonRequest.getJSONObject(OIDCCMConstants.JWKS)));
                        jsonRequest.remove(OIDCCMConstants.JWKS);
                        client.setServiceClient(true);
                        gotSupportedAuthMethod = true;
                    }
                    break;
    ...
    protected void handleResponseTypes(OA2Client client, JSONObject jsonRequest, OA2ClientKeys keys) {
        if (jsonRequest.containsKey(OIDCCMConstants.RESPONSE_TYPES)) {
            JSONArray responseTypes = toJA(jsonRequest, OIDCCMConstants.RESPONSE_TYPES);
        ...
        jsonRequest.remove(OIDCCMConstants.RESPONSE_TYPES);
    }
