...
public class OA2ServiceTransaction extends OA4MPServiceTransaction implements OA2TransactionScopes, OIDCServiceTransactionInterface, DateComparable {
    ...
    public String PROXY_HASH_KEY = "proxy_hash_key";
    ...
    public boolean isProxyAccessTokenComplete() {
        if (!getState().containsKey(PROXY_ACCESS_TOKEN_COMPLETE)) {
            return false;
        }
    ...
    /**
     * The proxy hash key is an internally managed unique identifier for consent pages
     * when OA4MP is using a proxy to do authentication.
     * @return
     */
    public String getProxyHash() {
        return getState().getString(PROXY_HASH_KEY);
    }

    public void setProxyHash(String proxyHash) {
        getState().put(PROXY_HASH_KEY, proxyHash);
    }

    public boolean hasProxyHash() {
        return getState().containsKey(PROXY_HASH_KEY);
    }
}
